window.RebelRanchPayments = {
  ...
  async startOneTimeCheckout(supabase, creatorId) {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      window.location.href = "account.html?returnTo=creation-station-live-classes.html";
      return;
    }

    const storageKey = `paypal-order-request:${creatorId}`;
    let requestId = sessionStorage.getItem(storageKey);
    if (!requestId) {
      requestId = crypto.randomUUID();
      sessionStorage.setItem(storageKey, requestId);
    ...
    const response = await fetch(
      "https://dfrwxpuojeiykaignyny.supabase.co/functions/v1/paypal-create-order",
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${session.access_token}`,
        ...
        body: JSON.stringify({ creator_id: creatorId, request_id: requestId }),
      },
    );
    const result = await response.json();
    if (!response.ok || !result.approve_url) {
  ...
  clearCheckoutAttempts() {
    for (const key of Object.keys(sessionStorage)) {
      if (key.startsWith("paypal-checkout-request:") || key.startsWith("paypal-order-request:")) {
