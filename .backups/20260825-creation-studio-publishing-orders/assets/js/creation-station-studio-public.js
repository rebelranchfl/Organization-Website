...
function cartSummaryText(){
  return cart.map(c=>`${c.qty}x ${c.title}${c.price_label?` (${formatPrice(c.price_label)})`:''}`).join(', ');
}
...
function renderCart(){
  ...
  box.innerHTML=`<ul class="cart-line-list">${cart.map(c=>`<li><span>${c.qty}&times; ${esc(c.title)}${c.price_label?` &mdash; ${esc(formatPrice(c.price_label))}`:''}</span><button type="button" data-remove-from-cart="${esc(c.id)}" aria-label="Remove ${esc(c.title)} from cart">&times;</button></li>`).join('')}</ul>`;
...
async function init(){
  ...
  const payMethods=(req.payment_methods||[]).map(m=>{
    ...
    if(m==='paypal'&&req.payment_link)return `<a href="${esc(req.payment_link)}" target="_blank" rel="noopener">PayPal — Pay Now ↗</a>`;
    const handle=req.payment_handles?.[m];
    return handle?`${esc(paymentLabels[m]||m)}: ${esc(handle)}`:esc(paymentLabels[m]||m);
  });
  $('studio-payment').innerHTML=payMethods.length?payMethods.map(m=>`<p>${m}</p>`).join(''):'<p>Contact for payment options.</p>';

  const delivery=(req.delivery_methods||[]).map(m=>esc(deliveryLabels[m]||m));
  $('studio-delivery').innerHTML=`<p>${delivery.length?delivery.join(', '):'Contact for delivery options.'}</p>`;
