...
async function renderActionQueue(force=false){
  ...
  try{
    ...
    const actions=[];
    reviews.forEach(x=>actions.push(actionCard(x,'Owner review',`${stageLabel(x.workflow_stage)} · ${x.progress_next||'Review and decide.'}`)));
...
function handleStageHash(){if(!isStage||!location.hash||handledHash===location.hash)return;const hash=location.hash;if(hash==='#owner-decision'){const target=document.querySelector('.asr-review');if(target){handledHash=hash;setTimeout(()=>scrollToEl(target),80)}}else if(hash==='#late-findings'){const wrap=document.querySelector('#academy-late-findings')?.closest('.aou-collapsible');if(wrap){wrap.open=true;handledHash=hash;setTimeout(()=>scrollToEl(wrap),80)}}else if(hash==='#review-content'){const target=stageJumpTarget('review');if(target){handledHash=hash;setTimeout(()=>scrollToEl(target),80)}}}
