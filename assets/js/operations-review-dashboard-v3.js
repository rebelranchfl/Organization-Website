...
const CORE_QUERY_TIMEOUT_MS=8000;
const OPTIONAL_QUERY_TIMEOUT_MS=4500;
...
function withTimeout(query,label,ms){
  ...
  return Promise.race([
    ...
    new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error(`${label} did not respond within ${Math.ceil(ms/1000)} seconds.`)),ms);})
  ]).finally(()=>clearTimeout(timer));
}
async function requiredQuery(query,label){
  const result=await withTimeout(query,label,CORE_QUERY_TIMEOUT_MS);
  if(result?.error)throw new Error(`${label}: ${result.error.message}`);
  return result?.data||[];
}
async function optionalQuery(query,label){
  try{
    const result=await withTimeout(query,label,OPTIONAL_QUERY_TIMEOUT_MS);
    if(result?.error)throw result.error;
    return{data:result?.data||[],warning:null};
  }catch(error){
    console.warn(`Operations Review optional data unavailable: ${label}`,error);
    return{data:[],warning:label};
  }
}
async function loadData(){
  const [projects,o,r,path,steps]=await Promise.all([
    requiredQuery(
    ...
    optionalQuery(
      ...
      'Audience pathway steps'
    )
  ]);
  const warnings=[o.warning,r.warning,path.warning,steps.warning].filter(Boolean);
  return{projects,opps:o.data,rels:r.data,paths:path.data,steps:steps.data,warnings};
}
...
function attentionRows(data){
  const rows=[];data.projects.filter(p=>p.current_status==='READY_FOR_REVIEW'&&['RESEARCH_REVIEW','PRODUCT_REVIEW','FINAL_PRODUCT_REVIEW'].includes(p.workflow_stage)).slice(0,4).forEach(p=>rows.push({title:`Review ${p.title}`,sub:`${p.project_id} is at ${stageLabel(p.workflow_stage)}.`,id:p.project_id}));
...
async function refreshOverview(){
  ...
  const old=document.querySelector('#orv3-overview');
  const top=document.querySelector('main>.topbar');
  const updated=document.getElementById('orv3-updated');
  if(old)old.innerHTML='<div class="orv3-card">Refreshing live Academy state…</div>';
  if(updated)updated.textContent='Loading live state…';
  try{
    const data=await loadData();
    old?.remove();
    if(!top)throw new Error('Operations Review header could not be found.');
    top.insertAdjacentHTML('afterend',overviewHtml(data));
    if(updated)updated.textContent=`Last updated: ${new Date().toLocaleString([], {month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}`;
    wireOverview();
  }catch(e){
    console.error('Operations Review v3',e);
    const errorHtml=`<section class="orv3-overview" id="orv3-overview"><div class="orv3-card"><strong>Operations Review could not load the core project state.</strong><p class="muted">${esc(e.message)}</p><p class="muted">Your data has not been changed. Use Refresh above to try again.</p></div></section>`;
    if(old)old.outerHTML=errorHtml;else if(top)top.insertAdjacentHTML('afterend',errorHtml);
    if(updated)updated.textContent='Live state unavailable — retry';
  }
}
