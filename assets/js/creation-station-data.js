...
export async function loadIdentity(){
  ...
  const results=await Promise.all([
    ...
    supabase.from('creator_profiles').select('id,display_name,public_name,creator_type,age_band,profile_status').eq('owner_user_id',user.id).eq('profile_status','active').order('created_at')
...
export async function loadWorkspace(identity){
  ...
  const results=await Promise.all([
    ...
    supabase.from('creator_projects').select('id,creator_id,template_id,title,status,completion,notes,is_favorite,started_at,completed_at,updated_at').eq('owner_user_id',uid).order('updated_at',{ascending:false}),
...
export async function loadAdminSummary(){
  const results=await Promise.all([
    supabase.from('creator_portfolios').select('id,review_status,owner_user_id,creator_id').in('review_status',['submitted','changes_requested']),
    supabase.from('creator_website_requests').select('id,status,brand_name,owner_user_id,creator_id,updated_at').in('status',['submitted','changes_requested']),
  ...
  ]);fail(results);return {portfolioQueue:results[0].data,websiteQueue:results[1].data,allProjects:results[2].data,allCreators:results[3].data};
}

export const actions={
  async startProject(identity,template,creatorId){return supabase.from('creator_projects').insert({owner_user_id:identity.user.id,creator_id:creatorId,template_id:template.id,membership_offer_code:identity.membership?.offer_code||'creator_website',title:template.title,status:'in_progress',completion:5,started_at:new Date().toISOString()})},
  ...
  async submitWebsite(identity,payload){return supabase.from('creator_website_requests').insert({...payload,owner_user_id:identity.user.id,status:'submitted',submitted_at:new Date().toISOString()})},
