...
async function visualSection(data,stageKey,rerender){
...
async function render(){
  const id=selectedId();if(!id)return;if(id!==lastProject){lastProject=id;cache.clear()}
  ...
  host.querySelectorAll('[data-stage]').forEach(b=>b.onclick=()=>{activeStageByProject.set(id,b.dataset.stage);render()});
  host.querySelector('#lsw-prev')?.addEventListener('click',()=>{activeStageByProject.set(id,STAGES[stageIndex(view)-1].key);render();host.scrollIntoView({behavior:'smooth',block:'start'})});
  host.querySelector('#lsw-next')?.addEventListener('click',()=>{activeStageByProject.set(id,STAGES[stageIndex(view)+1].key);render();host.scrollIntoView({behavior:'smooth',block:'start'})});
  ...
  if(view==='VISUAL_PRODUCTION'||view==='FINAL_PRODUCT_REVIEW'){
    const vs=await visualSection(data,'VISUAL_PRODUCTION',render);if(token!==renderToken)return;const slot=host.querySelector('#lsw-visual-slot');if(slot){slot.innerHTML=vs;wireVisual(host,data,render)}
  }
